...
open class YouTubePlayerView: UIView, UIWebViewDelegate {
    ...
    // MARK: Load player
    open func loadVideoURL(_ videoURL: String?) {
        if let videoURL = videoURL {
            let pattern = "((?<=(v|V)/)|(?<=be/)|(?<=(\\?|\\&)v=)|(?<=embed/))([\\w-]++)"
            guard let regExp = try? NSRegularExpression(pattern: pattern, options: .caseInsensitive) else {
                return
            }
            let nsLink = videoURL as NSString
            let options = NSRegularExpression.MatchingOptions(rawValue: 0)
            let range = NSRange(location: 0, length: nsLink.length)
            let matches = regExp.matches(in: videoURL as String, options:options, range:range)
            if let firstMatch = matches.first {
                let videoId = nsLink.substring(with: firstMatch.range)
                loadVideoID(videoId)
            }
        }
    }
